...
/**
 * .
 * ├── If node t has right subtree, nextInorderNode(t) is the leftmost leaf of its right subtree.   
 ...
 *              ├── until we find a upstream node t', for which t' is in parent(t')'s left subtree, then nextInorderNode(t) is parent(t'); 
 *              └── until for node t', for which parent(t') is null, then t is the last node in inorder sequence of the tree. 
 */
